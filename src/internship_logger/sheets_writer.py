...
def _get_client():
    ...
    if sa_path:
        # Service account flow (share your Sheet with the SA email)
        return gspread.service_account(filename=sa_path)
    # OAuth flow using client_secret + token
...
def upsert_row(ws, row: Dict[str, Any]) -> None:
    # Find if EmailId already exists
    cells = ws.findall(row["EmailId"]) if row.get("EmailId") else []
    if cells:
